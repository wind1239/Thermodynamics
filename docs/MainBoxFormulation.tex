...
\begin{equation}
  \frc{\partial\mu_{i}}{\partial x_{j}} = 
      ...
      \end{cases}
\end{equation}
The main issue is to calculate $\frc{\partial\ln\varphi_{i}}{\partial x_{j}}$ from either fundamental relations (Eqn.~\ref{Chapter:EOSPR:Eqn:FugacityCoeffDef}) or from parametrised PVT relations, i.e., EOS (see Section~\ref{Chapter:EOSPR:Section:FugacityCoefficients}).

%%%
%%% SECTION
%%%
\section{Phase Stability Analysis}\label{Chapter:ThermodynamicFormulation:Section:PhaseStabilityAnalysis}
This section aims to investigate the thermodynamic stability of multi-component and two-phase systems. 

Let's consider a system at prescribed temperature, $T$ and pressure, $P$, conditions has chemical species with $N_{1}, N_{2}, \cdots, N_{n_{c}}$ moles. For simplicity, let's first assume that the whole mixture is at vapour phase, and the Gibbs free energy of the system is,
\begin{displaymath}
   G_{0} \equiv \mfr[G]{}{V}\left(\mfr[N]{1}{V}, \mfr[N]{2}{V}, \cdots, \mfr[N]{n_{c}}{V}\right) = \summation_{i=1}^{n_{c}}\mfr[N]{i}{V}\mfr[\mu]{i}{V}.
\end{displaymath}   
...
\begin{displaymath}
   \mfr[\mu]{i}{V}\left(\mfr[N]{1}{V}, \mfr[N]{2}{V}, \cdots, \mfr[N]{n_{c}}{V}\right) = \mfr[\mu]{i}{V}\left(z_{1},z_{2}, \cdots, z_{n_{c}}\right),
\end{displaymath}
...
If $\mfr[x]{i}{V}=\frc{\mfr[N]{i}{V}-\mfr[\varepsilon]{i}{L}}{\summation_{i=1}{n_{c}}\left(\mfr[N]{i}{V}-\mfr[\varepsilon]{i}{L}\right)}$ and $\mfr[x]{i}{V}=\frc{\mfr[\varepsilon]{i}{L}}{\summation_{i=1}{n_{c}}\mfr[\varepsilon]{i}{L}}$ are the molar fraction of component $i$ in the vapour and liquid phases, respectively, then the Gibbs free energy of the system in the new equilibrium state is
\begin{equation}
   G_{1} \equiv \mfr[G]{}{V}\left(\mfr[N]{1}{V}-\mfr[\varepsilon]{1}{L}, \cdots, \mfr[N]{n_{c}}{V}-\mfr[\varepsilon]{n_{c}}{L}\right) + \mfr[G]{}{L}\left(\mfr[\varepsilon]{1}{L}, \cdots, \mfr[\varepsilon]{n_{c}}{L}\right),
\end{equation}   
with
\begin{equation}
...
and
\begin{equation}
   \mfr[G]{}{L}\left(\mfr[x]{1}{L}, \cdots, \mfr[x]{n_{c}}{L}\right) = \summation_{i=1}^{n_{c}}\mfr[\varepsilon]{i}{L}\mfr[\mu]{i}{L}\left(\mfr[x]{1}{L}, \cdots, \mfr[x]{n_{c}}{L}\right)
\end{equation}  
